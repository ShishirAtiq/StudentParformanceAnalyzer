...
@section('content')
      ...
      <div class="modal fade" id="modal-form" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-md" role="document">
            <form id="submittedForm">
                @csrf
                @include('backend.master-group.form_modal')
            </form>
          </div>
      </div>
  ...
  <script src="{{asset('/backend/custom-js/master_group.js')}}"></script>
  ...
  <script>
    ...
    function add (instituteList, instituteId, classList) {
      ...
      $('#mydropdown').text('');
      $("#exampleModalLabel").text("Group Entry");
    ...
    function edit (item, instituteList, classList) {
      $('#submittedForm').trigger("reset");
      $("#modal-form").modal('show');
      $('#submittedForm').append(`<input type="text" hidden name="id" value="${item.id}" id="id">`);
      $("#exampleModalLabel").text("Group Modify");
      $('#mydropdown').text('');
